...
Errores típicos:
- **`Communications link failure` con MySQL**: esperar a que MySQL termine de arrancar (su healthcheck debería gatillar `depends_on`, pero la primera vez puede tardar más).
- **`401` contra DUX**: el token está expirado. Regenerar `dux_tokens.json` desde el super-master.

### La impresora térmica se traba a la 3ª-4ª etiqueta

Es problema físico del cabezal (sobrecalentamiento o suciedad), **no del software**. Ver el comentario en [`etiquetas-page.ts`](showroom-frontend/src/app/showroom/etiquetas-page/etiquetas-page.ts) sobre el modo ZPL — es la solución para tiradas largas.

Si persiste: limpiar cabezal con alcohol isopropílico y bajar darkness/speed en el driver Zebra.

### El sync se quedó "en curso" para siempre

```bash
docker compose restart backend
```

El estado del sync se rehidrata desde la última fila de la tabla `sync_log` al arranque, así que si quedó marcado como en curso por una caída, el restart limpia el estado.
