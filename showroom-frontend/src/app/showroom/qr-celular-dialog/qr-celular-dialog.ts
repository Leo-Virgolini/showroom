...
/**
 * Diálogo "QR para celular" con tres códigos: WiFi (estático), el visor de la
 * página (dinámico, provisto por inputs) y la reseña de Google (estático).
 *
  ...
  template: `
    <p-dialog header="QR para celular" [visible]="visible()" (visibleChange)="visible.set($event)"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 md:divide-x md:divide-surface-200 dark:md:divide-surface-700">
        ...
        <!-- 2. Visor (dinámico, provisto por el padre) -->
        <div class="flex flex-col items-center gap-1.5 md:px-3 border-t pt-3 md:border-t-0 md:pt-0 border-surface-200 dark:border-surface-700">
          <div class="w-full flex items-center gap-1.5">
            ...
            <h3 class="text-sm font-bold text-[#3B1E09] dark:text-surface-0 m-0">2. {{ visorTitulo() }}</h3>
          </div>
          @if (qrDataUrl()) {
